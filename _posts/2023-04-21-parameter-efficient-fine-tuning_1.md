...
categories: tech
tags: [fine-tuning]
excerpt: "Large Language Model을 밀어서 잠금해제: Parameter-Efficient Fine-Tuning 1"
back_color: "#eff7ff"
toc: true
show: false
new: true
...
##### * 생성형 AI (Generative AI): 텍스트나 이미지, 소리 등의 창작 작업을 수행할 수 있는 AI 모델들을 폭넓게 부르는 말입니다.    
##### * 거대 언어 모델 (Large Language Model; LLM): 언어모델 (LM) 은 앞선 단어들을 보고 다음에 올 단어를 예측하도록 훈련된 모델을 통틀어 부르는 말입니다. 후술하겠지만 최근의 언어모델은 많이 커져서 LLM으로 불리게 되었습니다.   
##### * 정렬 ([AI] Alignment): 생성형 AI 분야에서 쓰일 때에는 사람의 의도를 따르도록 혹은 AI의 행동을 사람에게 안전한 방향으로 조정하는 것을 가리킬 때 사용합니다. 이 글에서는 전자의 의미로 사용하였습니다.
...
젠슨 황에게 GPT4에 대해 질문을 받자, 몇 가지를 설명하다 문득 이런 이야기를 합니다.  

>[NVIDIA GTC 2023 젠슨 황과의 대담 세션]
>
...
요컨데, 지금의 GPT4가 정말 신기하긴 하지만, 정말 신기한 점은 GPT4를 가능하게 한 그 모든 것들이 이미 딥러닝의 태동 (2012) 전부터 있었으며, 딥러닝을 시작시킨 아이디어에서 많이 벗어나지 않았다는 것입니다.  
...
뇌를 본 딴 신경망 구조는 1958년에 Frank Rosenblatt [프랭크 로젠블랏] 에 의해 Perceptron 이라는 이름으로 제안되었고 1985년 Geoffrey Hinton [제프리 힌튼] 이 이를 역전파 알고리즘*으로 학습시킬 수 있다는 것을 재조명하였습니다.  
지금도 딥러닝을 처음 배우는 사람은 이 둘 (구조와 학습알고리즘) 부터 배웁니다.  
사실상 딥러닝의 알파와 오메가이죠...

하지만 이 둘의 조합이 딥러닝이라는 이름으로 주목받기 시작한 것은 2012년, 3번째로 열린 이미지넷 챌린지였습니다.  
여기서 방대한 규모의 신경망인 AlexNet이 다른 모든 참가팀을 큰 차이로 압도하는 것을 시작으로 딥러닝은 큰 주목을 받게 되었습니다.

이를 시도할 당시 큰 신경망에 대한 이론적 힌트가 없지는 않았습니다[UAT]  
Parallel Distributed Processing (Rumelhart, 1987) 에서도 찾아볼 수 있는 이 아이디어는 현대에 와서는 GPU Computing으로 구현되었지만, 당시에 이 기술은 별로 알려져있지 않았었습니다.  
때문에 큰 신경망을 통해 많은 데이터를 학습한다는 생각은  성능대비연산효율이나 인프라 등의 문제로 별로 인기있는 아이디어는 아니었죠.
...
*Figure 1. AlexNet 이미지넷 챌린지 2012 결과: AlexNet이 2012년 모델들 뿐만 아니라 이전 1등들도 큰 격차로 이기고 있다 (error rate ↓)*
...
*Figure 3. 순환신경망의 작동방식: 왼쪽처럼 나온 아웃풋이 다음 입력에 영향을 주도록 작동한다. $$W, V, U$$ 순환신경망의 가중치이고 $$x, s, o$$는 각각 입력, 상태, 출력이다.*
...
이런 순환신경망의 문제는 2017년에 혜성과 같이 등장한 트랜스포머 (Transformer) 에서 해결됩니다.  
...
*Figure 5. (좌) GPT 사전학습, (우) 사전학습 후 번역을 위해 미세조정하는 경우, 미세조정 (학습) 방법. (단순한 진행을 위해 GPT방식의 사전학습만 다룹니다)*
...
*Figure 6. 트랜스포머가 등장한 2017년 직후 2018년부터 PLM 혹은 LLM의 크기는 해에 따라서 지수함수적으로 커진다. GPT-3는 175B.*
...
*Figure 7. In-context Learning (LLM = GPT-3 175B): 주어진 입력 맥락에서 배우는 것 같이 보여서 In-context Learning이라고 불린다.*

![]({{"/assets/img/post/1f6f252aef508cb69fbb05b893f2f2f27639d869/fig8.png"| relative_url}})
*Figure 8. Chain of Thought Prompting (LLM = PaLM 540 B): 꼬리를 무는 생각처럼 입력을 주면 질문에 더 잘 대답하는 현상*
...
*Figure 10. 왼쪽부터 T5, FLAN. 이미 T5에서는 적절히 여러 작업으로 multitask learning 을 진행하여 각각 작업에 미세조정한 것과 근접한 성능에 도달하는 모델을 만들 수 있음을 시도한 바 있다. FLAN의 경우, 처음 보는 지시도 잘 수행할 수 있음을 이어서 확인했다.*
...
여기에 Human Feedback을 직접적으로 반영하는 방법으로서의 강화학습이 합쳐져 현재의 ChatGPT의 근간을 이루게 되었습니다.  
...
하지만 소규모의 기업과 비영리단체들에서도 LLM을 잘 학습시켜 유사-ChatGPT들을 속속 내놓고 있습니다.  
이들이 갑자기 투자를 많이 받은 것도 아닐테고, 어떻게 이런 일이 가능할까요?  
여기에는 몇가지 이유가 있습니다.  

* Instruction Tuning는 소량의 Instruction dataset (<100k) 만을 필요로 한다.
* 공용 LLM은 많이 풀려있으며 \[llama], 위와 같은 효과를 내는데에 필요한 LLM의 최소사이즈는 GPT-3 (175B)의 1/30 정도 크기인 7B 사이즈인것으로 보인다.
...
2부에서는 가장 빈번히 쓰이고 있는 LoRA를 비롯한 Parameter Efficient Fine-Tuning 방법론들을 소개해드리겠습니다.  

감사합니다 좋은 하루 되세요.

=]

##### *ChatGPT의 기술적 배경에 대해 더 깊이 알고 싶은 분들은 이 글도 도움이 될 것 같습니다 ([링크](https://yaofu.notion.site/How-does-GPT-Obtain-its-Ability-Tracing-Emergent-Abilities-of-Language-Models-to-their-Sources-b9a57ac0fcf74f30a1ab9e3e36fa1dc1#3d2cea076da849a094da28f4b4fd528c))
