...
이러한 문제를 해결하기 위해서 LangChain은 여러 형태의 추상화된 인터페이스를 제공하고, 기본적으로 제공되는 구현을 사용하거나 혹은 자기가 원하는 정책을 직접 구현하여 연쇄적인 체인Chain의 한 부분으로 동작하게 할 수 있습니다. 또한 LangChain에는 챗봇 뿐만이 아니라, 연쇄적인 동작이 필요한 다른 시나리오도 해결하려고 하고 있습니다.
...
실제 LLM을 통한 구현이라면 `_call` 함수로 받은 prompt를 LLM에 넘기는 등의 동작을 하겠지만, 이 예제를 위해서 무조건 초기화 할 때 `reply`로 주어진 문자열을 반환하도록 Custom LLM을 작성하였습니다.
...
그런 처리를 쉽게 하기 위하여 히스토리를 `dict`로 변환하거나 `dict`에서 히스토리를 생성하는 내용은 [문서에 관련 내용](https://python.langchain.com/en/latest/modules/memory/getting_started.html#saving-message-history)이 있습니다.

<br/>

## 길이 줄이기

LLM과 나누는 대화가 길어졌을 때, LLM에 모든 대화가 들어가면 물론 이상적이겠지만 성능이나 비용의 문제로 입력의 길이에 어느정도 제한을 두는 것이 현실적입니다.
...
위의 출력 결과를 보면 프롬프트에는 Memory를 생성할 때 주어진 `k=3` 인자에 알맞게 가장 최근의 세 번의 대화만 입력되고 있는 것을 볼 수 있습니다.

하지만 마지막의 `pprint` 출력처럼 히스토리 자체는 모두 보관되는 것이 기본 동작입니다. 히스토리가 LLM에 직접 들어가지는 않더라도, 웹 화면 같은 곳에서 사용자가 볼 채팅 히스토리와 상태가 일치하도록 하는데 유용히 사용될 수 있습니다.
...
AgentExecutor 모듈은 특정한 형태로 조립된 Chain으로, 이미 특정한 형식의 숨겨진 Prompt를 갖고 있고, LLM의 답변 형식이 딱딱 들어맞기를 기대하여 이를 파싱하고, 그 파싱한 결과를 통해 사용자의 추가 입력 없이도 여러번 다시 LLM을 호출하여 답을 도출해냅니다.
...
Agent를 잘 설정하고 좋은 질문과 좋은 LLM만 있다면 질문 한 번으로 여러번의 Tool 호출을 거쳐 AI가 스스로 단계별로 생각하는 것처럼 여러 작업을 수행할 수 있습니다.

`AgentExecutor`(`initialize_agent`가 반환하는 값의 클래스입니다)는 각 단계마다 LLM이 반환하는 값을 파싱하여 필요한 도구들을 호출합니다.
...
3. Agent가 OpenAI의 답변을 파싱하여 사용하려는 도구에 인자를 넣고 실행해 Observation을 합니다.
...
기회가 된다면 OpenAI나 HuggingFace의 LLM을 사용하여 여러 기능을 테스트해보는 것도 즐거운 일이 되리라 생각합니다.
